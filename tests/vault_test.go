...
func TestTerraformHttpExample(t *testing.T) {
	...
	instanceName := fmt.Sprintf("terratest-http-vault-%s", uniqueID)
	...
	instanceURL := terraform.Output(t, terraformOptions, "elb_url")

	// It can take a while for the Instance to boot up, so retry a few times
	maxRetries := 30
